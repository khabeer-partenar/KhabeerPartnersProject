@if($departmentData)
    ...
    @if(auth()->user()->hasPermissionWithAccess('updateOrder'))
        <a class="btn btn-sm btn-default change_dept_order" data-backend-url={{ route('system-management.departments.updateOrder', $departmentData) }} data-action="up">
            <i class="fa fa-arrow-up" aria-hidden="true"></i>
        </a>
        

        <a class="btn btn-sm btn-default change_dept_order" data-backend-url={{ route('system-management.departments.updateOrder', $departmentData) }} data-action="down">
    ...
    @if(auth()->user()->hasPermissionWithAccess('departmentsAuthoritiesEdit'))
        <a href="{{ route('system-management.departments-authorities.edit', $departmentData) }}" class="btn btn-sm btn-warning">
